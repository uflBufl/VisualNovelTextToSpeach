...
| Model and voice assets | Download/verify a model and import a voice pack | Download, checksum verification, voice-pack import and character-reference import run in the background with visible progress, safe close and retry | The active manifest now uses a labelled field with Browse and explicit checksum Validate actions. Empty, changed, valid and invalid states remain inline; Save selects the invalid path instead of opening modal churn. Manifest controls freeze and recover with the existing operation lifecycle. Keyboard, accessibility and 680 x 440 compact states are covered without changing import ownership. | Complete |
...
| Dialogue history | Search, replay and export the current session | Live search, readable labelled detail, text/JSON export and async replay with progress, retry and deferred close; search and selected details expose explicit accessible names and label buddies; entering the modal uses an asynchronous live-capture stop barrier | Timer refresh preserves the exact older selection, details and scroll position while new entries arrive, without an intermediate deselection. Replay Stop is intentionally absent: the current controller exposes only shared emergency stop, not request-scoped cancellation for the exact history replay. | Complete |
...
| Game profiles | Create, duplicate, rename, remove and activate a profile | Small focused dialog with explicit active versus selected state, one primary `Use selected profile` action, a separate management group, destructive confirmation and prevention of active-profile removal; controller shutdown/start runs in a background transaction with visible busy/failure state and retryable profile action | A later visual pass may add profile icons or richer metadata, but activation and management semantics are now explicit without relying on color. | P3 |
