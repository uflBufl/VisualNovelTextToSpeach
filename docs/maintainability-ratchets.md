...
- imports of underscore-prefixed names from another `vntts` module;
...
code may shrink below that ceiling, but it may not grow; a new offender is
rejected even when it is smaller than the repository's largest legacy offender.
...
is removed, its now-unused baseline entry should be removed in the same change.
